...
```
Summary of SPRR experiments which actually reached steady state during the run
<!-- ## Run no. 1 no SS -->
...
```{r no. 1 data, eval=FALSE, include=FALSE}
nr_1 = read_delim("data/SPRR/gammel skrald/nr_1.csv",delim = "\t", escape_double = FALSE, 
...
flow_boolean = dat_1$flow0501>10&dat_1$flow0501<25
...
```{r no. 1 plot, eval=FALSE, message=FALSE, include=FALSE}
...
```{r no. 2 data, include=FALSE}
nr_2_a = read_delim("data/SPRR/nr_2_a.csv",delim = "\t", escape_double = FALSE, 
                  trim_ws = TRUE, skip = 5) #Hente data og få det til passe ned i en tabel
nr_2_b = read_delim("data/SPRR/nr_2_b.csv",delim = "\t", escape_double = FALSE, 
                  trim_ws = TRUE, skip = 5) #Hente data og få det til passe ned i en tabel
nr_2=rbind(nr_2_a,nr_2_b)

dat_2 = rbind(as.data.frame(nr_2_a),as.data.frame(nr_2_b)) #Ændrer strukturen af dataen til noget vi kan arbejde med
...
flow_boolean = dat_2$flow0501>10&dat_2$flow0501<25
start_tid = min(which(flow_boolean))
...
add_lines(x=~(sek/60/60),
  ...
  layout(title="Run no. 2",
         legend=list(x=0.9,y=0.9),
         yaxis=list(title="value ", range=c(NA,NA)),
         xaxis=list(title="Time [Hours]",range=c(NA,NA))
         ) #Her plottet TMP
...
Tryk_boolean = dat_3$p_avg0301>2.5
start_tid_tryk = min(which(Tryk_boolean))
dat_3=dat_3[-c(1:start_tid_tryk),]
flow_boolean = dat_3$flow0501>11&dat_3$flow0501<25
start_tid = min(which(flow_boolean))
...
flow_boolean = dat_5$flow0501>10&dat_5$flow0501<25
start_tid = min(which(flow_boolean))
...
add_lines(x=~(sek/60),
  ...
  layout(title="Run no. 5",
         legend=list(x=0.9,y=0.9),
         yaxis=list(title="value ", range=c(NA,NA)),
         xaxis=list(title="Time [min]",range=c(NA,NA))
         ) #Her plottet TMP
...
nr_6 = read_delim("data/SPRR/nr_6.csv",delim = "\t", escape_double = FALSE, 
                  trim_ws = TRUE, skip = 5) #Hente data og få det til passe ned i en tabel
dat_6_raw = as.data.frame(nr_6) #Ændrer strukturen af dataen til noget vi kan arbejde med
t0=693
dat_6 = dat_6_raw[-(1:t0),]
...
tid = dat_6$time = dmy_hms(nr_6$TimeStamp[t0:length(nr_6$TimeStamp)-t0]) #Vi ændrer formatet af 'dato' kolonnen til et standard format for tid og datoer istedet for at det bare er noget tekst
cut = T
if (cut==T){
  flow_boolean = dat_6$flow0501>10&dat_6$flow0501<25
  start_tid = min(which(flow_boolean))
  ...
  ## Tid i s fra forsøgs start
  sek=(seq(from = 0, to =  nrow(dat_6)-1))
  dat_6 = dat_6%>%mutate(sek=sek)
}
...
add_lines(x=~(sek/60),
  ...
  layout(title="Run no. 6",
         ...
         yaxis=list(title="value ", range=c(NA,NA)),
         xaxis=list(title="Time [min]",range=c(NA,NA))
         )%>%layout(hovermode = 'compare') #Her plottet TMP
...
```{r no. 7 data, eval=FALSE, include=FALSE}
...
flow_boolean = dat_7$flow0501>10&dat_7$flow0501<25
start_tid = min(which(flow_boolean))
...
```{r no. 7 plot, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
...
Tryk_boolean = dat_8_igen$p_avg0301>2.5
start_tid_tryk = min(which(Tryk_boolean))
dat_8_igen=dat_8_igen[-c(1:start_tid_tryk),]
flow_boolean = dat_8_igen$flow0501>10&dat_8_igen$flow0501<25
start_tid = min(which(flow_boolean))
...
#Tryk_boolean = dat_9$p_avg0301>2.5
#start_tid_tryk = min(which(Tryk_boolean))
#dat_9=dat_9[-c(1:start_tid_tryk),]
flow_boolean = dat_9$flow0501>10&dat_9$flow0501<25
start_tid = min(which(flow_boolean))
...
flow_boolean = dat_11$flow0501>10&dat_11$flow0501<25
start_tid = min(which(flow_boolean))
...
Tryk_boolean = dat_12$p_avg0301>2.5
start_tid_tryk = min(which(Tryk_boolean))
dat_12=dat_12[-c(1:start_tid_tryk),]
flow_boolean = dat_12$flow0501>10&dat_12$flow0501<25
start_tid = min(which(flow_boolean))
...
```{r no. 13 data, include=FALSE}
nr_13 = read_delim("data/SPRR/gammel skrald/nr_13.csv",delim = "\t", escape_double = FALSE, 
                  trim_ws = TRUE, skip = 5) #Hente data og få det til passe ned i en tabel
...
flow_boolean = dat_13$flow0501>10&dat_13$flow0501<25
start_tid = min(which(flow_boolean))
...
flow_boolean = dat_14$flow0501>10&dat_14$flow0501<25
start_tid = min(which(flow_boolean))
...
pH_boolean = dat_15$pH0201>9.5
start_tid_pH= min(which(pH_boolean))
dat_15=dat_15[-c(1:start_tid_pH),]


flow_boolean = dat_15$flow0501>10&dat_15$flow0501<25
start_tid = min(which(flow_boolean))
...
```{r no. 17 data, include=FALSE}
nr_17_a = read_delim("data/SPRR/nr_17_a.csv",delim = "\t", escape_double = FALSE, 
                  trim_ws = TRUE, skip = 5) #Hente data og få det til passe ned i en tabel
nr_17_b = read_delim("data/SPRR/nr_17_b.csv",delim = "\t", escape_double = FALSE, 
                  trim_ws = TRUE, skip = 5) #Hente data og få det til passe ned i en tabel
nr_17=rbind(nr_17_a,nr_17_b)

dat_17 = rbind(as.data.frame(nr_17_a),as.data.frame(nr_17_b))
...
flow_boolean = dat_17$flow0501>10&dat_17$flow0501<25
start_tid = min(which(flow_boolean))
...
flow_boolean = dat_18$flow0501>10&dat_18$flow0501<25
start_tid = min(which(flow_boolean))
