...
level_order = c('Na','Cl','SO4','SiO2','Ca','HCO3')
DE.long = store_df_DE %>%filter(type=="pauling")%>% 
  gather(key,value,Na,Cl,SO4,SiO2,Ca,HCO3)

ggplot(DE.long,aes(x=rp,y=value,color=factor(key,level=level_order)))+geom_line()+
  scale_color_brewer(palette="Set1",labels = level_order)+
...
ui <- fluidPage(
  sidebarLayout(
    sidebarPanel(
      fluidRow(
        column(width=6,
               sliderInput("r", "Pore Radius",0.5, 1, 0.5, step=0.005)
        )
      )
    ),
    mainPanel(
      selectInput("Var",
                  ...
                  choices = c("Steric"="U", "Dielectric"="V"), 
                  selected = c("U")
      ),
...
server <- function(input, output,session) {
  ...
  output$plot0<-renderPlotly({
   ...
   Data = Data%>%gather(key,value,Na,Cl,SO4,SiO2,Ca,HCO3)
  str_r=as.character(input$r)
  ggplotly(ggplot(Data, aes(fill=type, y=value, x=key)) + 
    geom_bar(position="dodge", stat="identity")+labs(title=paste("Dielectric exclusion for r =",  str_r,"nm"),
        x ="Ion", y = "Exclusion factor")+ylim(c(0,1)))
  })
