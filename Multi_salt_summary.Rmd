...
nice_plot = ggplot(data=Facet_data%>%filter(Stream=="Feed",Name=="ID:1"),aes(x=Recovery,y=Value,color=ion))+
  geom_point()+geom_line()+scale_color_brewer(palette= "Set1")+
    geom_errorbar(data=Facet_data%>%filter(Stream=="Feed",Name=="ID:1"),aes(x=Recovery,ymin=Value-(Value*Std), ymax=Value+(Value*Std),color=ion), width=1, position=position_dodge(0.05))+
    facet_grid(cols=vars(Name),row=vars(plot),scale="free")+labs(x="Water Recovery %",y="Concentration mM", color="Species")
