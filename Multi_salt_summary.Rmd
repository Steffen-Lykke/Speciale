...
print(Total_time_h <- tail(dat_M_9.2$sek, n=1)/3600)
print(mean(dat_M_9.2$flow0501))


flow0501_slid <- rollapply(dat_M_9.2$flow0501,60,mean,align="right",fill=NA)
ggplot()+
...
csumflow0501 <- cumsum((dat_M_9.2$flow0501)/60)
dat_M_9.2 <- cbind(dat_M_9.2,csumflow0501)
print(total_perm_volume <- tail(dat_M_9.2$csumflow0501, n=1))
print(8.8/10*100)


#ggplot()+
 # geom_line(data=dat_M_9.2, aes(x=sek/3600, y=csumflow0501/1000), shape= 16)+
  #labs(x="Time [h]",y=" Accumulation of Permeate [L]")+
  #ylim(0,9)+
  #ggtitle("Multi salt pH 9.2 Accumulate of Permeate ") 
...
```{R pH9.2 plot IC data, echo=FALSE, message=FALSE}
...
ggplot()+
  geom_point(data=pH9.2_IC_data, aes(x=Time, y=Silica, col=Stream), shape= 16)+
  geom_point(data=pH9.2_Feed[-1,], aes(x=pH9.2_Feed$Time[-1], y=pH9.2_rej_SiO2))+
  #geom_point(data=pH9.2_IC_data, aes(x=Time, y=Sodium, col=Stream), shape= 17)+
  #geom_point(data=pH9.2_Feed[-1,], aes(x=pH9.2_Feed$Time[-1], y=pH9.2_rej_Na))+
...
dat_M_9.2 %>%
  ...
  add_trace(x=~pH9.2_Permeate$Time,
            y=~pH9.2_Permeate$Conductivity,
            name='Conductivity perm',
            mode='markers',
...
```{R pH9.2 plot pH, echo=FALSE, message=FALSE}


ggplot()+
  geom_point(data=dat_M_9.2, aes(x=sek/3600, y=pH0201), shape= 16)+
  geom_point(data=pH9.2_Feed[-1,], aes(x=pH9.2_Feed$Time[-1], y=pH))+
  labs(x="Time [h]",y=" Concentration [mg/L]")+
  #ylim(0,135)+
  ggtitle("pH of multisalt pH 9.2")
...
```{R pH9.2 plot Bicarbonate data, echo=FALSE, message=FALSE}

pH9.2_Bi_data <- read_excel("data/Multi_salt_alle_change_pH.xlsx")

old_names_Bi=colnames(pH9.2_Bi_data) #Vi finder de latterlige navne kolonnerne i dataen har 
...
pH9.2_Bi_data = pH9.2_Bi_data %>% rename_at(vars(old_names_Bi),~new_names_Bi) #Vi ændrer navnene til de nye
pH9.2_Bi_data <- as.data.frame(pH9.2_Bi_data)
pH9.2_Bi_data <- pH9.2_Bi_data[-c(1:16),-c(5:12)] #cutter IC data fra så det kun er bicarboant.  

ggplot()+
  geom_point(data=pH9.2_Bi_data, aes(x=Time, y=Bicarbonate, col=Stream), shape= 16)+
  #geom_point(data=pH9.2_IC_data, aes(x=Time, y=Sodium, col=Stream), shape= 17)+
  #geom_point(data=pH9.2_Feed[-1,], aes(x=pH9.2_Feed$Time[-1], y=pH9.2_rej_Na))+
  labs(x="Time [h]",y=" Concentration [mg/L]")+
  #ylim(0,135)+
...
pH10_IC_data <- pH10_IC_data[-c(14:22),] #cutter bicarbonat af. 
