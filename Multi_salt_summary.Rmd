...
Measurements taken at various times from IC data
##### pH 9.2
...
C=data.frame(matrix(ncol=3,nrow=length(rejection)))
colnames(C)=c('time','key','value')
C$time=A$Time
C$key=A$key
C$value=rejection
...
ggplot(C, aes(x=time, y=value,color= key))+geom_point()+geom_line()+
