...
IC_9.2$Sulphate = replace_na(IC_9.2_raw$Sulphate,5)
...
colnames(C_9.2)=c('time','key','value')
C_9.2$time=A$Time
C_9.2$key=A$key
...
IC_10$Sulphate = replace_na(IC_10$Sulphate,5)
...
IC_10_C =data.frame(IC_9.2_B$Time,IC_9.2_B$Na,IC_9.2_B$Cl)
IC_10.5_C =data.frame(IC_10.5_B$Time,IC_10.5_B$Na,IC_10.5_B$Cl)


IC_9.2_C$cl_rejection[2:7] = all_ion_9.2%>%filter(key=="Chloride")%>%select(3)
...
colnames(IC_9.2_C) = colnames(IC_10_C) = colnames(IC_10.5_C) = c("time","% Na","% Cl","Cl Rejection")


IC_9.2_D = na.omit(IC_9.2_C)%>%gather(key = key, value = value,"Cl Rejection")
ggplot(IC_9.2_D, x = time, y = value, color = key )+geom_point()
