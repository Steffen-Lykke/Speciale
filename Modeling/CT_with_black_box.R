...
) #En vektor med de forskellige koncentrationer [mol/m3]
...
##### Model Parameters #####
dt=1 #tidsstep i timer
dt=dt/24 #timer i dage
...
V_NF2=V_NF1*0.77 #hvor meget vand der kommer tilbage afhægit af recovery

Q_vap = Q_vap*dt/24 #Hvor meget fordamper per tidsskridt !FIX!
...
while(i < n_time_step){
  ...
  c_NF=cf[i,2:6]*Rej[2,]
  nf[i,2:6] = nf[i-1,2:6]*(V_CT-V_NF1)+c_NF*V_NF2+(V_NF1-V_NF2)*c_makeup
  cf[i,2:6] = nf[i,2:6]/df$V_CT[i]  
