...
### noget med recovery
Ion_data_frame = Ion_data_frame%>%mutate("perm_vol"=time)
...
rej_9.2=ggplot(multisalt_plot%>%filter(type=="M_9.2"), aes(x=Recovery, y=value,color= key))+geom_point()+geom_line()+geom_hline(yintercept=0)+
  ...
  labs(x="Water Recovery",y="Conc", color="")+xlim(c(20,90))+ylim(c(-65,100))+
  ggtitle("pH 9.2")
...
```
```{r ICR}
ggplotly(ggplot(Ion_data_frame%>%filter(type %in% c("ICR")), aes(x=Recovery, y=Rej_Cl,color=type))+geom_point()+geom_line())
...
rej_ICR=ggplot(ICR_rej, aes(x=Recovery, y=value,color= key))+geom_point()+geom_line()+geom_hline(yintercept=0)+
  scale_color_brewer(palette= "Set1")+
  labs(x="Water Recovery",y="Conc", color="")+xlim(c(0,90))+ylim(c(-65,100))+
  ggtitle("ICR")
ggplotly(rej_ICR)
