...
CWF_CT_vand = read_delim("data/CT_vand_12.10.2021_del1.csv",delim = "\t", escape_double = FALSE, 
                  trim_ws = TRUE, skip = 5) #Hente data og få det til passe ned i en tabel
#str(CWF)
dat_CT = as.data.frame(CWF_CT_vand) #Ændrer strukturen af dataen til noget vi kan arbejde med
old_names=colnames(CWF_CT_vand) #Vi finder de latterlige navne kolonnerne i dataen har 
...
dat_CT = dat_CT %>% rename_at(vars(old_names),~new_names) #Vi ændrer navnene til de nye
dat_CT$time = dmy_hms(dat_CT$time)#Vi ændrer formatet af 'dato' kolonnen til et standard format for tid og datoer istedet for at det bare er noget tekst
