...
Sodium and Chloride data is presented together where Chloride is represented by circles and sodium represented by squares. 
...
ggplot()+
  geom_point(data=NaCl_IC_data, aes(x=time, y=Chloride, col=Stream), shape= 1)+
  #geom_point(data=NaCl_Feed[-1,], aes(x=NaCl_Feed$time[-1], y=NaCl_rej_Cl))+
  geom_point(data=NaCl_IC_data, aes(x=time, y=Sodium, col=Stream), shape= 0)+
  #geom_point(data=NaCl_Feed[-1,], aes(x=NaCl_Feed$time[-1], y=NaCl_rej_Na))+
  labs(x="Time [h]",y=" concentration [mg/L]")+
  ylim(0,135)+
  ggtitle("IC analysis, Concentration")
...
```
IC analysis was made for Calcium and Chloride. The results are presented in the following plot where Chloride is presented by circles and Calcium presented by squares. 
...
ggplot()+
  geom_point(data=CaCl2_IC_data, aes(x=time, y=Chloride, col=Stream), shape= 1)+
  #geom_point(data=CaCl2_Feed[-1,], aes(x=CaCl2_Feed$time[-1], y=CaCl2_rej_Cl))+
  geom_point(data=CaCl2_IC_data, aes(x=time, y=Calcium, col=Stream), shape= 0)+
  #geom_point(data=CaCl2_Feed[-1,], aes(x=CaCl2_Feed$time[-1], y=CaCl2_rej_Ca))+
  labs(x="Time [h]",y=" Concentration [mg/L]")+
  ylim(0,350)+
  ggtitle("Calcium concentration and rejection")
...
```{R Na2OSiO2 plot concentration, echo=FALSE, message=FALSE}
 #Na2OSiO2 data 
...
ggplot(data=SiO2, aes(x=tid))+
  geom_point(aes( y=SiO2_conc_feed, color="Feed"),shape= 0)+
  geom_point(aes( y=SiO2_conc_conc,color="Concentrate"),shape= 0)+
  geom_point(aes( y=SiO2_conc_permeate,color="Permeate"),shape= 0)+
...
The rejection of both the NaCl and the CaCl2 experiment is compared in the follow plot. 
The rejection in the CaCl2 experiment is significantly larger and increase slightly over the corse of the experiment, where it was expected that the rejection would increase as seen for the conductivity rejection, this might be due to the small no of data samples for IC data. The rejection is about 50-60% which is similar for the conductivity experiment. 

The rejection in the NaCl experiment is sinificantly lower than the CaCl2 experiment, and is more constant over the 4 hours. When comparing to the conductivity rejection which decrease from 50-25 % it is similar. 
The rejection of SiO2 is lower than both the rejection seen in the NaCl and CaCl2 experiment. The rejection of SiO2 is fluctuating around 13 %. The Sodium concentraiton has not been measured yet, but it is theorized that this will be much higher and match the conductivity rejection at 60-70%, as Sodium accounts for the conductivity in this experiment.   


It is interesting to compare the chloride rejection from the two experiment where there is a higher concentration in the CaCl2 (double) but also significantly higher rejeciton also double. 
...
There is a much higher rejection of Calcium than sodium, despite a similar concentraiton of 3mM, this difference is again contributed to the nature of the multivalent calcium ions. 
...
ggplot()+
  ...
  geom_point(data=SiO2,aes(x=tid, y=SiO2_rejection,color="R SiO2 "))+
  scale_color_manual(values=c("CaCl2 R Ca"="red", "CaCl2 R Cl"="blue", "NaCl R Na"="green", "NaCl R Cl"="orange", "R SiO2"="black"),labels=c("CaCl2 R Ca", "CaCl2 R Cl", "NaCl R Na", "NaCl R Cl", "R SiO2"))+
