...
Ion_data_frame = Ion_data_frame%>%mutate(Cl_SO4 = Cl_mmol/SO4_mmol)
...
)%>%layout(title = 'Silica Rejection',scene = list(xaxis=axx,yaxis=axy,zaxis=axz))
...
)%>%layout(title = 'Silica Rejection',scene = list(xaxis=axx,yaxis=axy,zaxis=axz)) %>% style(hovertemplate = paste('Rejection: %{z}',
                                                '<br> pH: %{x}',
                                                '<br> Cl/SO4:%{y}'
                                                )
                          )
...
###Different fittings ###
fit_Clrej_cl= lm(Rej_Cl~Cl_mmol, data = Ion_data_frame)
summary(fit_Clrej_cl)
ggplotRegression(fit_Clrej_cl)

fit_Clrej_anion= lm(Rej_Cl~anion_total, data = Ion_data_frame)
ggplotRegression(fit_Clrej_anion)
summary(fit_Clrej_anion)
...
###Without point where [Cl] is very low: ###
Ion_data_frame_no8=Ion_data_frame%>%filter(name!="SPRR_8")
fit_Clrej_cl_no8= lm(Rej_Cl~Cl_mmol, data = Ion_data_frame_no8)
ggplotRegression(fit_Clrej_cl_no8)

fit_Clrej_anion_no8= lm(Rej_Cl~anion_total, data = Ion_data_frame_no8)
ggplotRegression(fit_Clrej_anion_no8)

fit_Clrej_cl_anion_no8 = lm(Rej_Cl~Cl_anion, data = Ion_data_frame_no8)
ggplotRegression(fit_Clrej_cl_anion_no8)


### Without point where [SO4] is very high ###
Ion_data_frame_no1=Ion_data_frame%>%filter(name!="SPRR_1")
fit_Clrej_cl_no1= lm(Rej_Cl~Cl_mmol, data = Ion_data_frame_no1)
ggplotRegression(fit_Clrej_cl_no1)

fit_Clrej_anion_no1= lm(Rej_Cl~anion_total, data = Ion_data_frame_no1)
ggplotRegression(fit_Clrej_anion_no1)

fit_Clrej_cl_anion_no1 = lm(Rej_Cl~Cl_anion, data = Ion_data_frame_no1)
ggplotRegression(fit_Clrej_cl_anion_no1)



### Without both points ###
Ion_data_frame_both=Ion_data_frame%>%filter(name!="SPRR_1")%>%filter(name!="SPRR_8")
fit_Clrej_cl_both_so4= lm(Rej_Cl~SO4_mmol, data = Ion_data_frame_both)
fit_Clrej_cl_both_cl_anion= lm(Rej_Cl~Cl_anion, data = Ion_data_frame_both)

summary(fit_Clrej_cl_both_so4)
summary(fit_Clrej_cl_both_cl_anion)

ggplotRegression(fit_Clrej_cl_both_so4)
ggplotRegression(fit_Clrej_cl_both_cl_anion)

#### Cl rejection multiple regression ####
fit_Clrej_multi = lm(Rej_Cl~Cl_anion+Rej_SiO2, data = Ion_data_frame)
...
cooksD <- cooks.distance(fit_Clrej_multi)
influential <- cooksD[(cooksD > (3 * mean(cooksD, na.rm = TRUE)))]
influential
#Number 26 (SPRR 8) has cooks distance >3x avg, if we exclude it our model is now:

fit_Clrej_multi_cook = lm(Rej_Cl~SO4_mmol+SiO2_mmol+Cl_mmol, data = Ion_data_frame%>%filter(name!="SPRR_8"))
summary(fit_Clrej_multi_cook)
plot(x=predict(fit_Clrej_multi_cook),y=t(Ion_data_frame%>%filter(name!="SPRR_8")%>%select("Rej_Cl")),
     xlab='Predicted Values',
     ylab='Actual Values',
     main='Predicted vs. Actual Values',
     abline(a=0, b=1))
#plot(fit_Clrej_multi_cook)

#As It is the one with too low cl it may be ok

### Without the excluded points (1 and 8)
fit_Clrej_both_multi = lm(Rej_Cl~SO4_mmol+SiO2_mmol+Cl_mmol, data = Ion_data_frame_both)
summary(fit_Clrej_both_multi)
#plot(fit_Clrej_both_multi)
