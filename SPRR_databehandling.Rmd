...
fit_SiO2_pH2 = lm(Rej_SiO2~pH+SiO2_mmol, data = Ion_data_frame)
ggplotRegression(fit_SiO2_pH2)+ylab("SiO2 Rejection [%]")
#plot(fit_SiO2_pH2)
summary(fit_SiO2_pH2)
...
plot(x=predict(fit_SiO2_pH2), y=na.omit(Ion_data_frame$Rej_SiO2),
     xlab='Predicted Values',
     ylab='Actual Values',
...
fit_Clrej_multi = lm(Rej_Cl~SO4_mmol*Cl_mmol, data = Ion_data_frame)
...
influential <- cooksD[(cooksD > (3 * mean(cooksD, na.rm = TRUE)))]
influential
#Number 30 (SPRR 12) has cooks distance >3x avg, if we exclude it our model is now:

fit_Clrej_multi_cook = lm(Rej_Cl~Cl_anion+pH, data = Ion_data_frame%>%filter(name!="SPRR_1"&name!="SPRR_8"))
summary(fit_Clrej_multi_cook)
plot(x=predict(fit_Clrej_multi_cook),y=(Ion_data_frame%>%filter(name!="SPRR_1"&name!="SPRR_8")),
     xlab='Predicted Values',
     ylab='Actual Values',
     main='Predicted vs. Actual Values',
     abline(a=0, b=1))
...
#Which is a slightly better fit R2=0.79 vs 0.63, Can you just do that? who knows? probably not
