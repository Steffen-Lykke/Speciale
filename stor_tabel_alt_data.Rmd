...
```{r aww shit here we go again, eval=FALSE, include=FALSE}
...
Ion_data_frame_A=Ion_data_frame%>%filter(!name%in%c("ID:4B","ID:6B","ID:8B"))
...
fit_SiO2_pH = lm(Rej_SiO2~pH, data = Ion_data_frame%>%filter(!name%in%c("ID:4B","ID:6B","ID:8B")))
ggplotRegression(fit_SiO2_pH)+ylab("SiO2 Rejection [%]")
summary(fit_SiO2_pH)
...
```{r aww shit here we go again_2, eval=FALSE, include=FALSE}
...
library(corrplot)
Ion_data_frame_var_SPRR=na.omit(Ion_data_frame%>%filter(type=="SPRR")%>%select(-c(state,type,name,Ca,pH2,Na,Cl,SO4,SiO2_mgL,time,start_vol,perm_vol,Recovery)))
