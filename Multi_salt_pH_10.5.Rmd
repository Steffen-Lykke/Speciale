...
```{R pH 10.5 import data plot pressure, echo=FALSE, message=FALSE}
...
dat_M_10.5 %>%#dplyr arbejder indenfor 'dat' data framen
 ...
 # add_lines(y=~TMP_slid,
  #          name='TMP',
  #          color='#forestgreen')%>%
  layout(title="TMP Multi salt pH 10.5",
...
```{R Multi pH 10.5 permeate flow , echo=FALSE, message=FALSE}

print(Total_time_h <- tail(dat_M_10.5$sek, n=1)/3600)
print(mean(dat_M_10.5$flow0501))


flow0501_slid <- rollapply(dat_M_10.5$flow0501,60,mean,align="right",fill=NA)
ggplot()+
  geom_point(data=dat_M_10.5, aes(x=sek/3600, y=flow0501_slid), shape= 16)+
  labs(x="Time [h]",y=" Permeate Flow mL/min")+
  ylim(15,20)+
  ggtitle("Multi salt pH 10.5 Permeate flow ") 

csumflow0501 <- cumsum((dat_M_10.5$flow0501)/60)
dat_M_10.5 <- cbind(dat_M_10.5,csumflow0501)
initial_feed_volume <- 8.5
print(total_perm_volume <- tail(dat_M_10.5$csumflow0501, n=1))
print((total_perm_volume/1000)/initial_feed_volume*100)


#ggplot()+
 # geom_line(data=dat_M_10.5, aes(x=sek/3600, y=csumflow0501/1000), shape= 16)+
...
```{R pH10.5 plot IC data, echo=FALSE, message=FALSE}
...
pH10.5_IC_data <- pH10.5_IC_data[-c(16:19),] #cutter bicarbonat af. 

pH10.5_Permeate <- filter(pH10.5_IC_data, Stream=="Permeate")
pH10.5_Feed <- filter(pH10.5_IC_data, Stream=="Feed")
   

#pH10.5_rej_Cl <- (1-(pH10.5_Permeate$Chloride/(pH10.5_Feed$Chloride[-1])))*100
#pH10.5_rej_Na <- (1-(pH10.5_Permeate$Sodium/(pH10.5_Feed$Sodium[-1])))*100
#pH10.5_rej_Ca <- (1-(pH10.5_Permeate$Calcium/(pH10.5_Feed$Calcium[-1])))*100
#pH10.5_rej_SO4 <- (1-(pH10.5_Permeate$Sulphate/(pH10.5_Feed$Sulphate[-1])))*100
pH10.5_rej_SiO2 <- (1-(pH10.5_Permeate$Silica/(pH10.5_Feed$Silica[-1])))*100

ggplot()+
  geom_point(data=pH10.5_IC_data, aes(x=Time, y=Silica, col=Stream), shape= 16)+
  geom_point(data=pH10.5_Feed[-1,], aes(x=pH10.5_Feed$Time[-1], y=pH10.5_rej_SiO2))+
  #geom_point(data=pH10.5_IC_data, aes(x=Time, y=Sodium, col=Stream), shape= 17)+
  #geom_point(data=pH10.5_Feed[-1,], aes(x=pH10.5_Feed$Time[-1], y=pH10.5_rej_Na))+
...
```{R pH 10.5 plot conductivity, echo=FALSE, message=FALSE}
ay <- list(
...
dat_M_10.5 %>%
  ...
  layout(title="Conductivity and rejection Multi salt pH 10.5",yaxis2=ay,
...
```{R pH10.5 plot pH, echo=FALSE, message=FALSE}


ggplot()+
  geom_point(data=dat_M_10.5, aes(x=sek/3600, y=pH0201), shape= 16)+
  labs(x="Time [h]",y=" Concentration [mg/L]")+
  #ylim(0,135)+
  ggtitle("pH of multisalt pH 10.5")
...
```{R pH10.5 plot Bicarbonate data, include= FALSE, echo=FALSE, message=FALSE}

pH10.5_Bi_data <- read_excel("data/Multi_salt_alle_change_pH.xlsx",3)

old_names_Bi=colnames(pH10.5_Bi_data) #Vi finder de latterlige navne kolonnerne i dataen har 
new_names_Bi=c("Time","Stream","Name","Bicarbonate","Chloride","Sulphate","Calcium","Silica","DIT_M_Cl") #Vi finder på nogle lidt bedre nogle
pH10.5_Bi_data = pH10.5_Bi_data %>% rename_at(vars(old_names_Bi),~new_names_Bi) #Vi ændrer navnene til de nye
pH10.5_Bi_data <- as.data.frame(pH10.5_Bi_data)
pH10.5_Bi_data <- pH10.5_Bi_data[-c(1:16),-c(5:9)] #cutter IC data fra så det kun er bicarboant.  

ggplot()+
  geom_point(data=pH10.5_Bi_data, aes(x=Time, y=Bicarbonate, col=Stream), shape= 16)+
  #geom_point(data=pH10.5_IC_data, aes(x=Time, y=Sodium, col=Stream), shape= 17)+
  #geom_point(data=pH10.5_Feed[-1,], aes(x=pH10.5_Feed$Time[-1], y=pH10.5_rej_Na))+
  labs(x="Time [h]",y=" Concentration [mg/L]")+
  #ylim(0,135)+
